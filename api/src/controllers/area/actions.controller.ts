...
export class ActionsController {
  ...
  @Post('spotify')
  @ApiOperation({ summary: 'Trigger if there is new spotify song in like playlist' })
  @ApiResponse({
    status: 201,
    description: 'success, spotify song getted',
  })
  @ApiBody({
    description: 'Webhook endpoint of the action discord guild.',
    schema: {
  ...
  async getSpotify(@Res() res: Response, @Req() request: Request): Promise<Response> {
    try {
      const [statusCode, message] = await this.actionsService.getSpotify(request.body);
